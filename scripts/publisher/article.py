...
from ..config import (
    BASE_DIR, ARTICLE_DIR, SITE_URL, SITE_NAME,
    CAT_MAP, TAG_TO_CAT, AFFILIATE_PRODUCTS
)


# HTML Template (using inline for now - should be in templates/ folder ideally)
HTML_TEMPLATE = """<!doctype html>
<html lang="ja">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{{ column_title }} — {{ site_name }}</title>
<meta name="description" content="{{ column_lead }}">
<link rel="canonical" href="{{ site_url }}/article/{{ slug }}/">
<meta property="og:title" content="{{ column_title }} — {{ site_name }}">
...
<header class="masthead" role="banner">
  <div class="container">
    <p class="masthead-date">{{ date }} 📝 {{ site_name }}・コラム 第{{ episode }}回</p>
...
{{ column_body_html }}
      ...
      <nav class="related-articles" style="margin-top:32px;padding-top:24px;border-top:1px solid var(--border);" aria-labelledby="related-heading">
        ...
        <div style="display:flex;gap:12px;flex-wrap:wrap;">
          ...
          <a href="/article/{{ r.slug }}/" class="related-card" style="flex:1;min-width:140px;background:var(--paper);border:1px solid var(--border);border-radius:var(--radius);padding:12px;text-decoration:none;color:inherit;transition:all .2s;" onmouseover="this.style.borderColor='var(--primary)';this.style.boxShadow='var(--shadow-md)'" onmouseout="this.style.borderColor='var(--border)';this.style.boxShadow='none'">
            <span style="display:inline-block;font-size:.65rem;font-weight:700;color:var(--primary);background:var(--primary-light);padding:2px 8px;border-radius:4px;margin-bottom:6px;">{{ r.cat_name }}</span>
            <span style="font-size:.8rem;font-weight:600;line-height:1.5;">{{ r.title }}</span>
          </a>
          {% endfor %}
        </div>
      </nav>
...
def build_article_page_direct(
    ...
    # Determine category from slug or default
    cat_name = cat_name or "健康"
...
def save_article(html: str, slug: str):
    """記事HTML保存"""
...
def determine_category(paper) -> str:
    """論文タグからカテゴリ決定"""
...
def update_search_index(column_data: dict, slug: str, paper):
    """検索インデックスに記事追加"""
    try:
        ...
        new_article = {
            ...
            "description": column_data["lead"],
            "category": determine_category(paper),
            "tags": column_data.get("affiliate_keywords", []),
            "date": paper.pub_date,
            "url": f"/article/{slug}/"
        }
